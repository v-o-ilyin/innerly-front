...
export function HowItWorks() {
  return (
    <section className="py-20 px-6 bg-gradient-to-b from-brand-50 to-white" id="how">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-sm font-semibold text-brand-500 uppercase tracking-widest">
            Как это работает
          </span>
          <h2 className="text-3xl lg:text-4xl font-bold text-stone-900 mt-2 mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {steps.map((s) => (
            <div
              key={s.step}
              className="bg-white rounded-2xl border border-brand-100 shadow-sm p-5 flex flex-col gap-4"
            >
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold text-brand-400 tabular-nums">{s.step}</span>
                <span className="text-2xl">{s.emoji}</span>
                <h3 className="font-bold text-stone-800 text-sm leading-tight">{s.title}</h3>
              </div>
                  ...
                  <div
                    ...
                    className={`rounded-xl px-3 py-1.5 text-[11px] leading-snug max-w-[90%] ${
                      m.mine
                        ...
                        : 'bg-amber-50 text-stone-700 rounded-bl-none'
                    }`}
                  >
                    {m.text}
                  </div>
